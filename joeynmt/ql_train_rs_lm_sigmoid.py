...
from joeynmt.ql_search_rs_lm_sigmoid import transformer_greedy, calculate_bleu, push_sample_to_memory
    ...
    for i_episode in range(outer_epochs):
        ...
        for i, batch in enumerate(iter(train_iter)):  # joeynmt training.py 377
            ...
            for t in range(inner_epochs):
                ...
                Q_value_sigmoid = torch.sigmoid(Q_value)
                ...
                Q_hat_value_sigmoid = torch.sigmoid(Q_hat_value)

                if use_cuda:

                    Q_hat_value_sigmoid = Q_hat_value_sigmoid.cuda()
                    mini_reward = mini_reward.cuda()
                    mini_is_eos = mini_is_eos.cuda()

                yj = mini_reward.float() + Gamma * Q_hat_value_sigmoid
                #print('yj', yj)
                index = mini_is_eos.long()
                #print('mini_is_eos', mini_is_eos)
                yj[index] = mini_reward[index]
                #print('yj', yj)
                #print('Q_value_sigmoid', Q_value_sigmoid)
                ...
                # Compute loss
                loss = -loss_function(yj, Q_value_sigmoid)
                print('loss', loss)
